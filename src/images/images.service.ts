...
export class ImagesService {
  ...
  async generateBatch(
    ...
    const referenceImageUrls =
      input.referenceImageUrls || referenceImage?.urls || null;
    const images: Image[] = [];
    ...
    return {
      images,
      requestId,
      chargedCredits: user.role === 'admin' ? 0 : quantity,
    };
  }
  ...
  private refundFailedImage(
    userId: number,
    requestId: string,
    imageId: number,
  ) {
    return this.usersService.addCredits(
      userId,
      1,
      'refund',
      '生成失败退还 1 积分',
  ...
  async getDownload(id: number, userId: number) {
    const image = await this.imagesRepository.findOne({ where: { id } });
    if (!image || image.userId !== userId || !image.imageUrl) {
      throw new NotFoundException('图片不存在');
    }

    const file = await this.minioService.openImageByUrl(image.imageUrl);
    const extension =
      file.key.match(/\.([a-z0-9]+)$/i)?.[1]?.toLowerCase() || 'png';
  ...
  async deleteImage(id: number, userId: number): Promise<void> {
    ...
    if (image.imageKey) {
      try {
        ...
        this.logger.warn(
          `Unable to remove stored image ${image.id}: ${String(error)}`,
        );
      }
    }
